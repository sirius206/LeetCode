...
where k-th bucket contains all numbers in [min + (k-1)gap, min + k*gap). Since there are n-2 numbers that are not equal min or max and there are n-1 buckets, 
...
//2. Radix sort time O(N) space O(N) 
