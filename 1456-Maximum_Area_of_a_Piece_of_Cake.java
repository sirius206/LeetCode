...
// sort, then find (maxWidth * maxHeight
class Solution {
    ...
    public int maxArea(int h, int w, int[] horizontalCuts, int[] verticalCuts) {
        ...
        long maxHeight = Math.max(horizontalCuts[0], h - horizontalCuts[n - 1]);   // long, also use Math.max instead of creating a gap array
        for (int i = 1; i < n; i++) {
